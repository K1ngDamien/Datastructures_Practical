...
public class Customer implements Comparable<Customer>{
    ...
    public Customer(LocalTime queuedAt, String zipCode) {
        ...
        // DONE: initialize an empty set of purchased items
    }
    ...
    public int getNumberOfItems() {
        int numItems = 0;
        // DONE: Calculate the total number of items

        for (Purchase purchase : this.items) {
            numItems = numItems + purchase.getAmount();
        }
    ...
    public double calculateTotalBill() {
        ...
        for (Purchase purchase : this.items) {
            totalBill = totalBill + (purchase.getAmount() * purchase.getProduct().getPrice());
        }
        // DONE: Calculate the total cost of all items
    ...
    public Cashier selectCashier(List<Cashier> cashiers) {
        ...
        int checkOutTime = 20 + (2 * this.getNumberOfItems());

        if (cashiers.size() == 1) {
            selectedCashier = cashiers.get(0);
        } else {
            for (Cashier cashier: cashiers) {
                int passThroughTime = checkOutTime + cashier.expectedWaitingTime(this);

                if (selectedCashier == null) {
                    selectedCashier = cashier;
                } else {
                    if (passThroughTime < selectedCashier.expectedWaitingTime(this) + checkOutTime) {
                        selectedCashier = cashier;
                    }
                }
            }
            ...
            // DONE find the cashier with the lowest expected pass-through time.
            //  passthrough time = waiting time + time to check-out my own bought items
            //  waiting time = remaining time for the current customer +
            //          check-out times of all other customers that will be in front of me in the cashier's queue
    ...
    public int compareTo(Customer otherCustomer) {
        return otherCustomer.getZipCode().compareTo(this.getZipCode());
    }
}