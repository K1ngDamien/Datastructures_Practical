...
//         DONE create empty data structures for products and customers
    }
    ...
    public int getTotalNumberOfItems() {
        ...
        // DONE: calculate the total number of shopped items
    ...
    public void printCustomerStatistics() {
        ...
        // DONE: display the calculated revenues and most bought products.
        //ToDo Fix ,

        double totalRevenue = 0.0;
        for (Map.Entry<String,Double> revenueMap: revenues.entrySet()) {

            System.out.println(String.format("%s:%f(%s),", revenueMap.getKey(), revenueMap.getValue(), populars.get(revenueMap.getKey())));

            totalRevenue = totalRevenue + revenueMap.getValue();
        }
    ...
    public void printSimulationResults() {
        ...
        for (Cashier cashier : this.cashiers) {
            ...
            for (Customer customer : this.customers) {
                totalCheckoutTime = totalCheckoutTime + cashier.expectedCheckOutTime(customer.getNumberOfItems());
            }
            ...
            System.out.printf("%s          %s           %s          %s            %s               %s              %s", cashier.getName(), cashier.getTotalCustomers(), cashier.getAverageWaitingTime(), cashier.getMaxWaitingTime(), cashier.getMaxQueueLength(),  (totalCheckoutTime / this.customers.size()), cashier.getTotalIdleTime());
    ...
    public Map<String, Double> revenueByZipCode() {
        ...
        // DONE create an appropriate data structure for the revenues
        ...
        for (Customer customer: this.customers) {
            Double revenue = revenues.getOrDefault(customer.getZipCode(), 0.00);

            for (Purchase purchase: customer.getItems()) {
                revenue = revenue + (purchase.getProduct().getPrice() * purchase.getAmount());
            }
            revenues.put(customer.getZipCode(), revenue);
        }
    ...
    public Map<String, Product> mostBoughtProductByZipCode() {
        ...
        Map<String, List<ProductCount>> postalcodeProductCounts = new TreeMap<>();

        for (Customer customer : this.customers) {
            List<ProductCount> productsByZip = postalcodeProductCounts.getOrDefault(customer.getZipCode(), new ArrayList<>());
            for (Purchase purchase : customer.getItems()) {
                boolean equals = false;

                for (ProductCount productCount: productsByZip) {
                    if(productCount.product.equals(purchase.getProduct())) {
                        productCount.count = productCount.count + purchase.getAmount();
                        equals = true;
                    }
                }

                if (!equals) {
                    ProductCount newProduct = new ProductCount(purchase.getProduct());
                    newProduct.setCount(purchase.getAmount());
                    productsByZip.add(newProduct);
                }
            }

            postalcodeProductCounts.put(customer.getZipCode(), productsByZip);
        }



        for (Map.Entry<String, List<ProductCount>> productCount : postalcodeProductCounts.entrySet()){
            if (productCount.getValue().size() > 0) {
                List<ProductCount> sortedProducts = productCount.getValue();
                sortedProducts.sort(Collections.reverseOrder());

                mostBought.put(productCount.getKey(), sortedProducts.get(0).product);
            }
        }
    ...
    public void simulateCashiers() {
        ...
        // DONE: create an appropriate data structure for the shoppingQueue
        //  and add all customers in the supermarket
        ArrayList<Customer> sortedCustomers = new ArrayList<>();
        sortedCustomers.addAll(this.customers);
        Collections.sort(sortedCustomers, new SortCustomersByQueuingTime());
        ...
        // DONE: get the first customer from the shoppingQueue;
        ...
        while (nextCustomer != null) {
            ...
            // DONE: next customer is arriving, get the next customer from the shoppingQueue
